...
\[
 e'' + (k_2 + c)e' + k_1e = 0.
\]
This is a second-order linear ODE. The central question is how the control input, through the feedback parameters, affects stability and damping behavior.
...
The forces acting on the body are the thrust force $T(t)$, the gravitational force $-mg$, and the linear drag force $-bv(t)$. By Newton's second law~\cite{arnold_classical_mechanics}, we have
\[
 m h''(t) = T(t) - mg - bv(t).
\]
...
We introduce a function that measures the size of the error state over time and whose derivative along the controlled dynamics is non-positive \cite[Sec.~5.3, pp.~59--61]{yanushevsky2019modern}. Such a quantity should satisfy several requirements.

First, it should measure both the tracking error $e(t)$ and the error velocity $e'(t)$ in the CLE equation.

Second, it should be nonnegative and vanish only at the equilibrium state $e = e' = 0.$

Third, its derivative should contain the factor $(e'' + k_1e)$, so that the CLE equation turns it into a dissipative term.

These considerations lead to the choice
\[
 \boxed{E(t) = \frac{1}{2}(e'(t))^2 + \frac{1}{2}k_1e(t)^2.}
\]
We call this \textit{error state energy}. The first term measures the velocity part of the error, while the second term measures the position part of the error. Thus, at the level of the formula, this energy indicates how large the error state is at time $t$.
\begin{lemma}\label{decreasing}
  For all $t > 0$, we have $E'(t) = -\gamma (e'(t))^2$.
\end{lemma}
\begin{proof}
...
The lemma shows that the error energy is non-increasing in time. Hence the CLE system does not create new error energy; instead, the damping term $\gamma e'$ dissipates it.
...
The needed facts for the algebraic operator manipulations are stated and proved in Appendix~\ref{appendix:operator-algebra}.
...
\subsection{Comparison of Variational and Conventional Solutions of CLE}
...
\]
is the trajectory induced by the variational weights $p, q, r$. The case $k_2 = 1$ has weak damping and overshoots below the desired height. The case $k_2 = 4$ approaches $h_d$ more smoothly. The case $k_2 = 8$ is more heavily damped and remains above $h_d$ for a longer time. 
\begin{center}
...
The notation used in this model is summarized in Table~\ref{tab:fuel-notation}.
...
\end{tikzpicture}
\caption{Fuel burn and fuel dumping reduce the aircraft mass during flight.}
\label{fig:aircraft-fuel-dumping}
\end{figure}
Fuel burn is normally the main weight change during flight; as fuel is used, the aircraft becomes lighter and its performance is improved \cite{stevens_lewis_johnson_aircraft_control_2015}.
...
\[
 c(t) = \frac{b}{m(t)}.
\]
Thus mass loss directly changes the strength of the drag term.
\subsection{Energy Method in the Aircraft Model}

When the coefficient $c$ varies with time, the CLE equation becomes
...
\]
Choosing $D_x = \mathcal{E}_x(0)$, we obtain the desired estimate.
\end{proof}
...
We consider the idealized case where gravity and drag are neglected during the burn.
\begin{lemma}
...
\end{proof}
\subsection{Planetary Escape Velocity}
...
First, one may replace the linear drag law by a nonlinear dissipative term. Consider
\[
 h'' = u - g - c(t)|h'|h'.
\]
The corresponding error equation becomes
\[
...
\]
In finite time, for some observation window $\Delta T>0$, the sustained crossing condition is
\[
...
Let $I \subseteq\mathbb R$ be an interval. We write
\[
 D = \frac{d}{dt}.
\]
...
Suppose $f$ is sufficiently smooth and of exponential order so that the following transforms exist. Then
\[
